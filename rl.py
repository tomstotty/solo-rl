...
def ppo_train_minibatch_value_clip(
    ...
    """小批量 PPO 训练（策略更新后作截断值函数更新），返回 h/v 表、步表与
    各回合小批目标值。

    参数校验沿 ppo_train_value_clip；batch_size 须为非 bool 的正 int，
    shuffle_seed 须为非 bool 的 int（可为负），类型不符抛 TypeError，
    batch_size 非正抛 ValueError，全部参数在采样前校验完毕。状态编号、
    H/V 初始化、softmax 采样、GAE 轨迹、回合边界与每步仅一次
    random.Random(seed).random() 均沿 ppo_train_value_clip。第 e 回合
    （0 起）先以步序 [s, a, old_logp, A] 批次调用
    ppo_minibatch(H, batch, batch_size, float(alpha), c, epochs,
    shuffle_seed+e)，以其返回 logits 续训 H；shuffle_seed+e 只控制该
    回合洗牌，采样随机流不额外消费。再以按步序的 [s, 旧 v, R] 批次
    （R=A+采样时记录的旧 v）调用
    ppo_value_update(V, batch, float(value_lr), value_clip, epochs)，
    以其返回 values 续训 V。两次更新调用均透传异常，无部分结果。

    返回键依次为 h、v、episodes、objectives；h、v、episodes 结构沿
    ppo_train_value_clip，objectives 为各回合 ppo_minibatch 返回的
    完整 objectives（逐回合的小批目标列表）。同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
