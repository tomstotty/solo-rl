...
def _is_finite_number(value):
    """int 恒为有限；math.isfinite 对过大的 int 会抛 OverflowError。"""
...
def q_learning(env, episodes=500, alpha=0.5, gamma=0.9, epsilon=0.1, seed=0,
    ...
    random.Random(seed)。epsilon_end 为 None 时各回合探索率恒为
    epsilon；否则第 e 回合（0 起）线性插值至 epsilon_end。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    if epsilon_end is not None and (
        ...
        raise TypeError("epsilon_end must be None, an int or a float")
    if episodes <= 0:
    ...
    for episode in range(episodes):
        if epsilon_end is None or episodes == 1:
            ...
            rate = epsilon + (epsilon_end - epsilon) * episode / (episodes - 1)
        state = env.reset()
        for _ in range(1000):
            if rng.random() < rate:
