...
def emphatic_actor_critic(
    ...
    V[s]+=beta*ρ*M*δ。F、M 溢出或非有限，以及 ρ、δ、梯度中间量或
    任一更新值非有限即抛 ValueError。done 即停，否则置 s,ρprev=s2,ρ。
    全部随机性来自一个 random.Random(seed)。
...
def emphatic_ac_until(
    ...
    """强调离策略 actor-critic 训练至 H/V 快照收敛或达回合上限。

    训练完全等同 emphatic_actor_critic(env, behavior, interest,
    episodes, alpha, beta, gamma, lambda_, seed, max_steps)：同一
    random.Random(seed)，每步仅调用一次 random()，done 不自举，
    达步限截断的末步照常自举，F/M/ρ/δ 与 H/V 更新次序一致。env、
    behavior、interest、episodes、alpha、beta、gamma、lambda_、
    seed、max_steps 的校验均同 emphatic_actor_critic；check_every
    与 patience 须为非 bool 的 int，policy_tol、value_tol 须为非
    bool 的 int/float；错型抛 TypeError。check_every、patience
    非正抛 ValueError；policy_tol、value_tol 转 float 溢出、非
    有限或为负抛 ValueError。全部校验在首次 reset 前完成。

    快照为键序恰为 h、v 的 dict，行结构沿用 emphatic_actor_critic
    返回表：训练前先存一份全零表；此后每 check_every 回合及最终
    回合完成后各存一份当前 H/V 的独立副本，每次存入即调用
    actor_critic_convergence(snapshots, policy_tol, value_tol,
    patience)；首次 converged 为真即停止训练，否则跑满 episodes
    回合。F、M 溢出或非有限，以及 ρ、δ、梯度中间量或任一更新值
    非有限即抛 ValueError，且不返回部分结果。

    返回键依次为 h、v、episodes、snapshots、report；实际完成 k
    回合时，前三项逐值等于 emphatic_actor_critic 同参 k 回合的
    对应结果，report 为末次 actor_critic_convergence 的完整结果。
    同参同 seed 逐值一致。仅用标准库，不引入命令行入口。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
