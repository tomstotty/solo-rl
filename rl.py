...
def ppo_minibatch_kl(
    ...
    logits、batch、lr、c、batch_size、epochs、seed 的校验与异常，以及
    洗牌、切批、随机消费均逐项沿用 ppo_minibatch；target_kl 须为非
    bool 的 int/float，类型不符抛 TypeError，转 float 溢出、非有限或
    <=0 抛 ValueError，校验后记为 t。全部参数先校验完毕且不修改输入，
    随后仅建一个 random.Random(seed)，复制 logits 为 L。

    每轮按 ppo_minibatch 的 Fisher–Yates 洗牌并切为小批，依次对每个
    小批调用 ppo_update(L, sub, lr, c, 1)，以返回的 logits 续训，并按
    顺序收集各小批 objectives[0]（float）。轮末按 batch 原序，以当前
    L 按 ppo_update 的稳定 log-softmax 重算各样本所选动作的新 logp；
    从 0.0 顺序累加 old_logp-new_logp，除以样本数得 float KL 并收集；
    任一新结果非有限抛 ValueError。若 KL>t，保留本轮全部更新并立即
    结束（stopped=True），否则至多执行 epochs 轮。返回的 logits 为最终
    L，objectives 为展平的各小批目标列表，probabilities 为最终 L 的
    稳定 softmax 二维 float list，kls 为逐轮 KL 的 float list，stopped
    为是否早停的 bool；返回新容器且不修改输入，同参同 seed 逐值一致。
    """
    if not isinstance(logits, list):
        raise TypeError("logits must be a list")
    if not logits:
