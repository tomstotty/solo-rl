...
def gae_actor_critic(
    ...
    """GAE(λ) actor-critic，整回合采样后统一更新，返回 h/v 表与统计。

    H 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，V 覆盖同样的格子，
    初始均为 0.0。每回合 reset，至 done 或 max_steps 步；每步按
    softmax(H[s,·]) 采样动作，仅调用一次 random()，并记录
    (s, a, r, p, v, v2)：v 为采样时 V[s]，done 时 v2=0，否则为当时
    V[s2]（步限截断仍自举）。回合末置 A=0，逆序作
    delta=r+gamma*v2-v、A=delta+gamma*lambda_*A 并保存；再正序以所存
    p、A 同时对各 b 作 H[s,b]+=alpha*A*(I[b=a]-p[b])，随后
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            value = v[state]
            next_state, reward, done = env.step(action)
            value2 = 0.0 if done else v[next_state]
            trajectory.append(
                (state, action, reward, prob, value, value2)
            )
            total_reward += reward
            if done:
                break
        ...
        advantage = 0.0
        for t in range(len(trajectory) - 1, -1, -1):
            _st, _action, reward, _prob, value, value2 = trajectory[t]
            delta = reward + gamma * value2 - value
            advantage = delta + gamma * lambda_ * advantage
            advantages[t] = advantage
        for t, (st, action, _reward, prob, _value, _value2) in enumerate(
            trajectory
        ):
