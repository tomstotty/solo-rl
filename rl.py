...
def ppo_train_normalized(
    ...
    """优势规范化的 PPO 训练（每回合 GAE 采样后多轮截断更新），返回
    h 表、逐回合步表与目标值。

    状态编号、采样、GAE、终止/截断、随机消费及返回的 h、episodes
    均逐项沿用 ppo_train；除 epsilon 外各参数的校验与异常逐项沿用
    ppo_train，全部校验在采样前完成。epsilon 须为非 bool 的
    int/float，类型不符抛 TypeError，转 float 溢出、非有限或 <=0
    抛 ValueError。第 e 回合以步序 [s, a, old_logp, A] 为 batch，
    调用 ppo_normalized_update(H, batch, float(alpha), c, epochs,
    epsilon)，以其返回 logits 续训；V 仍按原优势 A 与该步记录值 v
    作 V[s]+=alpha*(A+v-V[s])，规范化优势不用于 V 或额外采样。
    返回键依次为 h、episodes、objectives；objectives 为各回合
    ppo_normalized_update 返回的 objectives。新运算结果非有限均抛
    ValueError，失败无部分结果；同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
