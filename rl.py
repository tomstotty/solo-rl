...
def ppo_train_minibatch(
    ...
    """PPO 训练（每回合 GAE 采样后小批量多轮截断更新），返回 h 表、
    逐回合步表与目标值。

    状态编号、采样、GAE、终止/截断、V 更新及返回结构均逐项沿用
    ppo_train；batch_size 须为非 bool 的正 int，类型不符抛
    TypeError，非正抛 ValueError，全部校验在采样前完成。第 e 回合
    （0 起）以步序 [s, a, old_logp, A] 为 batch，调用
    ppo_minibatch(H, batch, batch_size, float(alpha), c, epochs,
    seed+e)，以其返回 logits 续训；seed+e 仅控制该回合洗牌，不额外
    消费采样随机流。返回键依次为 h、episodes、objectives；
    objectives[e] 为该回合 ppo_minibatch 返回的完整 objectives
    ...
    for episode in range(episodes):
        ...
        result = ppo_minibatch(
            h, batch, batch_size, float(alpha), c, epochs, seed + episode
        )
        h = result["logits"]
        all_objectives.append(result["objectives"])
