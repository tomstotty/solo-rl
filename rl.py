...
def natural_actor_critic(
    ...
    """自然梯度演员-评论家训练（每回合 GAE 后单次自然梯度更新），返回 h/v 表与步表。

    同名参数校验沿 ppo_train；damping 沿 natural_gradient，须为非
    bool 的 int/float，转换为 float 后有限且大于 0，错型抛
    TypeError，转换溢出或取值非法抛 ValueError。全部校验在首次
    reset 前完成。状态编号、H/V 零初值、URDL 采样、单一随机流及
    终止/截断沿 ppo_train。每回合记录 (s, a, r, v, v2)：v 为采样时
    V[s]，done 时 v2=0.0，否则为当时 V[s2]（步限截断仍自举）。回合
    末自 A=0.0 逆序递推 d=r+gamma*v2-v、A=d+gamma*lambda_*A；再将
    按步序的 [s, a, float(A)] 批次一次传给
    natural_gradient(H, batch, float(alpha), float(damping), 1e-10, 100)，
    以其返回 logits 续训 H；随后按步序作 V[s]+=alpha*(A+v-V[s])，
    式中 v 为该步记录值。natural_gradient 抛出的异常原样透传，无
    部分结果。

    返回键依次为 h、v、episodes；h 行、v 行、episodes 步结构沿
    ppo_train_value_clip 前三项。同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    for _ in range(episodes):
        ...
        for (s, _a, _r, value, _v2), adv in zip(trajectory, advantages):
            new_value = values[s] + alpha * (adv + value - values[s])
            if not math.isfinite(new_value):
                raise ValueError("updated value must be finite")
            values[s] = new_value
