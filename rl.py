...
def expected_sarsa(
    ...
    也照常选择下一动作并自举。done 时目标为 r 且不选下一动作；
    否则令 g 为 s2 上 URDL 首个最大 Q 动作，其概率为
    1-epsilon+epsilon/4，其余各为 epsilon/4，自 0.0 按 URDL 累加
    v=Σp(a)*Q[s2,a]，目标为 r+gamma*v。全部随机性来自一个
    random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            if done:
                ...
                expected = 0.0
                for a in actions:
                    prob = greedy_prob if a == greedy else other_prob
                    expected += prob * q[(next_state, a)]
