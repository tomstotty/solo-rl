...
def true_online_sarsa_lambda(
    ...
    Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    每回合 reset，资格迹 E 清零，q_old=0.0，先选动作；单回合最多
    max_steps 步，末步未终止也照常选择下一动作、自举并更新后截断。
    每步先记 q=Q[s,a] 再 step；done 时 q2=0.0 且不再选动作，否则
    立即选 a2 并记更新前 q2=Q[s2,a2]。令 delta=r+gamma*q2-q，按键序
    先 E[k]*=gamma*lambda_，再 E[s,a]+=1-alpha*E[s,a]；随后同步作
    Q[k]+=alpha*(delta+q-q_old)*E[k]，再 Q[s,a]-=alpha*(q-q_old)。
    任一次写入所得 Q 值非有限即抛 ValueError。done 即停，否则
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            if done:
                ...
                next_action = choose_action(next_state)
                q2 = q[(next_state, next_action)]
            delta = reward + gamma * q2 - q_sa
            ...
            if done:
                break
            state, action, q_old = next_state, next_action, q2
    return q
