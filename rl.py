...
def _trace_converge_history(paths):
    """加载多份 trace-converge 清单并按 M1 seed 序汇总历史趋势。

    全部清单均完整加载、轨迹全部读取、汇总完成后才生成输出字符串；
    window 须相同，threshold/tolerance/minimum 转 float 后
    float.hex() 须相同，seed 集合须相同（各清单内重复 seed 已由
    清单契约拒绝），趋势按 M1 的 seed 序对齐。任一违约抛
    ValueError。
    """
    ...
    base_seeds = [seed for seed, _ in reports[0]["groups"]]
    base_seed_set = set(base_seeds)
    for report in reports[1:]:
        if {seed for seed, _ in report["groups"]} != base_seed_set:
            raise ValueError("seed sets must match between manifests")
    ...
    episodes_by_seed = [
        {seed: group_report["episode"] for seed, group_report in report["groups"]}
        for report in reports
    ]
    trends = []
    for seed in base_seeds:
        episodes = [episodes[seed] for episodes in episodes_by_seed]
        changes = []
...
def _run(argv):
    ...
    if command == "run" and len(argv) == 4:
        ...
        sys.stdout.buffer.write(
            json.dumps(out, separators=(",", ":")).encode("utf-8") + b"\n"
        )
        return 0
    ...
    if command == "trace-converge" and len(argv) == 3:
        try:
            ...
            return 2
        sys.stdout.buffer.write(out.encode("ascii") + b"\n")
        return 0
