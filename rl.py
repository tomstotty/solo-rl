...
def ppo_line_search(
    ...
    """按几何衰减学习率试更新的 PPO 线搜索，返回固定键序 logits、
    kls、accepted、lr 的 dict。

    logits、batch、lr、c 的校验与异常逐项沿用 ppo_update；target 须为
    非 bool 的 int/float，类型不符抛 TypeError，转 float 溢出、非有限
    或 <=0 抛 ValueError，校验后记为 t；steps 须为非 bool 的 int 且属于
    [1, 61]，类型不符抛 TypeError，越界抛 ValueError。全部参数先校验
    完毕且不修改输入。

    随后按 i=0..steps-1 令 rate=lr/(2**i)，每次均从原 logits 调用
    ppo_update(logits, batch, rate, c, 1) 得候选，候选之间不串接；以
    ppo_policy_kl 计算原 logits 到候选 logits 的 mean KL，将该 float
    依序加入 kls，首个 KL<=t 即接受并停止。任一 rate 或 KL 非有限抛
    ValueError 且无部分结果。接受时 logits 取该候选的新矩阵、accepted
    为 True、lr 为 rate；全部拒绝时 logits 为原矩阵的新副本、accepted
    为 False、lr 为 None；kls 为已尝试的 KL float 列表。不修改输入，
    同输入逐值一致。
    """
    if not isinstance(logits, list):
        raise TypeError("logits must be a list")
