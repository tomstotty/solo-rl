...
def ppo_train_grad_clip(
    ...
    """梯度范数裁剪的 PPO 训练（每回合 GAE 采样后多轮裁剪更新），返回
    h 表、逐回合步表、目标值、裁剪前范数与缩放因子。

    状态编号、H/V 初始化、采样、GAE、V 更新、终止/截断、随机消费及
    h、episodes 返回逐项沿用 ppo_train；除 max_norm 外参数校验及异常
    逐项沿用 ppo_train，全部校验在采样前完成。max_norm 须为非 bool 的
    int/float，类型不符抛 TypeError，转 float 溢出、非有限或 <=0 抛
    ValueError。每回合以优势 A 按步序构造 [s, a, old_logp, A] 批次，
    调用 ppo_grad_clip(H, batch, float(alpha), c, epochs, max_norm)，
    以其返回 logits 续训；该调用不消费随机数。

    返回键依次为 h、episodes、objectives、norms、scales；后三项各与
    回合数等长，第 e 项为第 e 回合 ppo_grad_clip 返回的同名完整
    float 列表。新运算结果非有限均抛 ValueError，失败无部分结果；
    同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    for _ in range(episodes):
        ...
        result = ppo_grad_clip(
            h, batch, float(alpha), c, epochs, max_norm
        )
        h = result["logits"]
        all_objectives.append(result["objectives"])
