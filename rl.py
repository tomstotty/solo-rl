...
def q_sigma(
    ...
    Q 取 s2 上 URDL 首个最大动作 g；g 的概率为
    1-epsilon+epsilon/4、其余各 epsilon/4，自 0.0 按 URDL 累加
    v=Σp(b)*Q[s2,b]，目标为
    r+gamma*(sigma*Q[s2,a2]+(1-sigma)*v)（sigma=1 即 SARSA，
    sigma=0 即 Expected SARSA）；done 时目标为 r 且不再选下一动作。
    全部随机性来自一个 random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
        ...
        for _ in range(max_steps):
            ...
            if done:
                ...
                for a in actions:
                    ...
                    v += p * q[(next_state, a)]
                target = reward + gamma * (
                    sigma * q[(next_state, next_action)]
                    + (1 - sigma) * v
                )
            key = (state, action)
            new_value = q[key] + alpha * (target - q[key])
