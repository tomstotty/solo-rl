...
def ppo_train_line_search(
    ...
    """PPO 训练（每回合 GAE 采样后以线搜索选择学习率），返回 h 表、
    逐回合步表与各回合线搜索结果。

    状态编号、H/V 初值、采样、轨迹记录、GAE、终止/截断、随机消费及
    V 更新逐项沿用 ppo_train。每回合以步序 [s, a, old_logp, A] 批次
    调用 ppo_line_search(H, batch, float(alpha), c, target, steps)；
    接受则续用其返回 logits，拒绝则 H 不变，V 仍用原 A 更新。全部
    随机性来自一个 random.Random(seed)，不额外消费随机数。

    target、steps 的校验与异常沿用 ppo_line_search，其余同名参数
    沿用 ppo_train（不设 epochs），全部校验在采样前完成。被调接口
    异常原样透传且失败无部分结果。

    返回键依次为 h、episodes、kls、accepted、lrs；h、episodes 沿用
    ppo_train；kls、accepted、lrs 与回合数等长，依次为该回合
    ppo_line_search 返回的 KL 浮点列表、接受布尔值、采用的学习率
    浮点或 None。同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    for _ in range(episodes):
        ...
        result = ppo_line_search(h, batch, float(alpha), c, target, steps)
        if result["accepted"]:
            h = result["logits"]
