...
def qr_q(
    ...
    risk 选择动作评分且不改变 Z 原序："mean" 为分位值按原序从 0.0
    求和除 N；"lower"/"upper" 先将该动作分位值升序/降序复制，令
    w=level*N、m=floor(w)、f=w-m，分子为前 m 项从 0.0 累加并在
    f>0 时加 f 倍第 m 项（0 起），再除 w。当前动作与非终止下一动作
    均取评分最大且 URDL 首个。任一动作均值、风险评分或最终 q 值
    非有限时在 step 或返回前抛 ValueError。
    ...
    if risk not in ("mean", "lower", "upper"):
        raise ValueError("risk must be one of mean, lower, upper")
    if isinstance(level, bool) or not isinstance(level, (int, float)):
        raise TypeError("level must be an int or float")
    try:
        level = float(level)
    except OverflowError:
        raise ValueError("level is too large to convert to float")
    ...
    def score(state):
        """按 risk 返回各动作的评分（URDL 顺序），非有限即抛 ValueError。

        均值与风险评分均在此校验，确保 step 前发现非有限值；不修改
        Z 中各分位值的原序，仅对排序副本累加。
        """
        scores = []
        for act in actions:
            values = z[(state, act)]
            mean = sum(values, 0.0) / N
            if not math.isfinite(mean):
                raise ValueError("Z mean must remain finite")
            if risk == "mean":
                value = mean
            else:
                ordered = sorted(values, reverse=(risk == "upper"))
                w = level * N
                m = math.floor(w)
                f = w - m
                numerator = 0.0
                for idx in range(m):
                    numerator += ordered[idx]
                if f > 0:
                    numerator += f * ordered[m]
                value = numerator / w
            if not math.isfinite(value):
                raise ValueError("risk score must remain finite")
            scores.append(value)
        return scores

    def greedy_from_scores(scores):
        return actions[max(range(len(actions)), key=scores.__getitem__)]

    def greedy_action(state):
        return greedy_from_scores(score(state))
    ...
    for _ in range(E):
        ...
        for _ in range(M):
            ...
            if rng.random() < e:
                ...
                action = greedy_from_scores(scores)
            next_state, reward, done = env.step(action)
            if done:
                targets = [reward] * N
            else:
                best = greedy_action(next_state)
...
def visit_q(
    ...
    每步以 n=visits[s] 计算
    rate=end+(e-end)/(1+n/d)**p；n/d、1+n/d 或幂溢出/非有限（含
    计算溢出），或 rate 非有限时，在 visits[s] 自增与 step 前抛
    ValueError，随后 visits[s]+=1。消费一次
...
def q_evaluate_trace(
    ...
    """按给定 Q 表做 epsilon 贪心评估，返回逐回合轨迹与成功率。

    env、episodes、max_steps、seed 同名参数的校验与异常分类沿用
    q_learning_trace；epsilon 须为非 bool 的有限 int/float 且在
    [0, 1]，错型抛 TypeError，值错抛 ValueError。q 须为 dict 且键域
    与 q_learning 返回的 Q 相同（从 S 可达的非 G 格与 U/R/D/L 的
    全部组合），值须为有限 float；容器、键型或值型错抛 TypeError，
    键域不符或值非有限抛 ValueError；不修改 q。全部校验先于首次
    reset。

    全部随机性来自一个 random.Random(seed)：每步先取一次 random()，
    小于 epsilon 才再取一次 randrange(4) 按 URDL 选动作，否则取 q
    值最大且 URDL 序首个的动作。每回合 reset，到达 G 即停；到达
    步限截断时保留末步且末步 done=False。

    返回键序为 episodes、success_rate 的 dict：episodes 为回合列表，
    每项为键序 trace、total_reward、success 的 dict；trace 的步结构
    与类型同 q_learning_trace，total_reward 为 int 回报和，success
    ...
    for key, value in q.items():
        if (
            ...
            or len(key[0]) != 2
            or any(isinstance(v, bool) or not isinstance(v, int) for v in key[0])
            or not isinstance(key[1], str)
        ):
            raise TypeError("q keys must be ((row, col), action) tuples")
    ...
    if set(q) != expected_keys:
        raise ValueError("q must cover exactly the q_learning key domain")
...
def dyna_q_plus(
    ...
    同名参数的 TypeError/ValueError 合法域与 dyna_q 完全一致；
    kappa 须为非 bool 的 int/float，否则抛 TypeError，溢出、非有限
    或小于 0 时抛 ValueError。

    Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    每回合 reset，done 即停；单回合最多 max_steps 步。time 初值 0。
    每次处理状态 s 前，按 URDL 将尚未建模的动作以
    model[s,a]=(s, 0, False) 插入模型并置 last[s,a]=0。随后与
    dyna_q 相同地消费一次 rng.random() 决定是否探索，探索时再按
    URDL 调用 rng.randrange(4)，否则取 URDL 中首个最大 Q 动作。
    真实 step 后 time += 1，按 dyna_q 作真实更新，再以实际转移
    覆盖模型（保持插入序）并置 last[s,a]=time。

    之后规划 planning_steps 次：按模型插入序以
    rng.randrange(len(model)) 抽键 (ps, pa)，令
    b=kappa*sqrt(time-last[ps,pa])；done 时目标为 reward+b，
    否则为 reward+b+gamma*maxQ(next)，再更新 Q。b、目标或新 Q
    溢出/非有限时抛 ValueError。全部随机性来自一个
    random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    try:
        ...
        raise ValueError("kappa is too large to convert to float")
    ...
    for _ in range(episodes):
        state = env.reset()
        for _ in range(max_steps):
            # 处理状态 s 前，按 URDL 补齐未建模动作。
            for a in actions:
                pending = (state, a)
                if pending not in model:
                    model[pending] = (state, 0, False)
                    last[pending] = 0
            if rng.random() < epsilon:
                action = actions[rng.randrange(4)]
            else:
            ...
            for _ in range(planning_steps):
                ...
                if plan_done:
                    ...
                    plan_target = (
                        ...
                        + bonus
                        + gamma
                        * max(q[(plan_next, a)] for a in actions)
                    )
                if not math.isfinite(plan_target):
                    raise ValueError("target must be finite")
...
def bonus_q(
    ...
    """访问次数塑形奖励的 Q 学习，返回键序 q、counts 的 dict。

    Q、N 域序同 replay_q_learning 的 q：覆盖从 S 可达的非 G 格与
    U/R/D/L 的全部组合，键序为坐标升序 × URDL，初始分别为 0.0、0，
    两表互相独立。每回合 reset，done 即停；单回合最多 max_steps 步，
    到达步限截断时末步 done=False 仍照常自举更新。

    每个真实步先且仅调用一次 rng.random()，小于 epsilon 时再按 URDL
    调用 rng.randrange(4) 探索，否则取 URDL 中首个最大 Q 动作。step
    后先 N[s,a]+=1，令 b = bonus / N[s,a]**power；done 时目标为
    reward+b，否则目标为 reward+b+gamma*max_a Q[next,a]，随后
    Q[s,a] += alpha*(目标-Q[s,a])。b、目标或新 Q 非有限即抛
    ...
    if not math.isfinite(bonus) or bonus < 0.0:
        raise ValueError("bonus must be finite and non-negative")
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            try:
                ...
                raise ValueError("bonus value must remain finite")
            ...
            new_q = q[key] + alpha * (target - q[key])
            if not math.isfinite(target) or not math.isfinite(new_q):
                raise ValueError("Q value must remain finite")
            q[key] = new_q
            if done:
                break
...
def soft_q_learning(
    ...
    """最大熵（soft Q-learning）表格控制，返回 {"q": Q, "policy": pi}。

    q 与 policy 均为以 ((row, col), action) 为键、URDL 同键序的独立
    dict；Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    policy 各值为最终 Q 按该温度稳定 softmax 得到的概率。
    每回合 reset，单回合最多 max_steps 步，末步未终止仍按 soft 值自举。
    全部随机性来自一个 random.Random(seed)。

    每步按 URDL 令 m=max_a Q[s,a]、w[a]=exp((Q[s,a]-m)/temp)、
    z=sum(w,0.0)，取 u=rng.random()*z，从 0.0 按 URDL 累加 w 并
    选择首个累计值严格大于 u 的动作，未命中取 L。step 后 done 时
    目标为 r；否则在 next 状态按 URDL 令 m=max_a Q[s2,a]、
    z=sum(exp((Q[s2,a]-m)/temp),0.0)、soft=m+temp*log(z)，
    目标为 r+gamma*soft，作 Q[s,a]+=alpha*(目标-Q[s,a])；
    exp 下溢为 0.0 合法，m、z、soft、目标或新 Q 溢出/非有限抛
    ValueError。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            if done:
                ...
                next_m = max(q[(next_state, a)] for a in actions)
                next_w = [
                    math.exp((q[(next_state, a)] - next_m) / temp)
                    for a in actions
                ]
                next_z = sum(next_w, 0.0)
                if not math.isfinite(next_m):
                    raise ValueError("soft value max must remain finite")
                if not math.isfinite(next_z) or next_z <= 0.0:
                    raise ValueError("soft value partition function must be finite and > 0")
                soft = next_m + temp * math.log(next_z)
                if not math.isfinite(soft):
                    raise ValueError("soft value must remain finite")
                target = reward + gamma * soft
            if not math.isfinite(target):
                raise ValueError("target must remain finite")
            key = (state, action)
            new_value = q[key] + alpha * (target - q[key])
    ...
    policy = {}
    for policy_state in sorted(_reachable_cells(env)):
        if env._cell(policy_state) == "G":
            continue
        m = max(q[(policy_state, a)] for a in actions)
        w = [math.exp((q[(policy_state, a)] - m) / temp) for a in actions]
        z = sum(w, 0.0)
        if not math.isfinite(m):
            raise ValueError("policy max must remain finite")
        if not math.isfinite(z) or z <= 0.0:
            raise ValueError("policy partition function must be finite and > 0")
        for a, weight in zip(actions, w):
            probability = weight / z
            if not math.isfinite(probability):
                raise ValueError("policy probability must remain finite")
            policy[(policy_state, a)] = float(probability)
...
def differential_actor_critic(
    ...
    """在线一步差分（平均回报）actor-critic，恰执行 steps 个环境步。

    H/V 表的覆盖状态、零初值与 softmax(H[s,·]) 稳定采样均沿
    actor_critic：每步仅消费一个 random.Random(seed) 的 random()。
    另置标量平均回报 R=0.0。step 得 (s2, r, done)：
    done 时 δ=r-R-V[s]（终止不自举），否则 δ=r-R+V[s2]-V[s]
    （截断照常自举）。以采样时的旧概率 p 对 URDL 各 b 同步作
    H[s,b]+=alpha*δ*(I[b=a]-p[b])，再作 V[s]+=beta*δ、R+=eta*δ。
    done 或单回合一满 max_steps 即记录 [steps, reward, done] 并
    reset；steps 步耗尽时未落盘的末段同样记录。δ、H/V/R 更新值或
    更新后 H/V/R 非有限即抛 ValueError。

    返回键依次为 h、v、average_reward、episodes；h/v/episodes 结构
    沿 actor_critic，average_reward 为最终 R（float）。同参同 seed
    逐值一致，全部随机性来自一个 random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    for _ in range(steps):
        ...
        for b, p_b in zip(actions, probs):
            ...
            new_h_values[b] = new_h
        new_v = v[state] + beta * delta
        if not math.isfinite(new_v):
            raise ValueError("V value must remain finite")
        new_average_reward = average_reward + eta_value * delta
        if not math.isfinite(new_average_reward):
            raise ValueError("average reward must remain finite")
        for b in actions:
            h[(state, b)] = new_h_values[b]
        v[state] = new_v
    ...
    if episode_steps > 0:
        episode_results.append([episode_steps, total_reward, False])
...
def differential_gae(
    ...
    """在线差分（平均回报）actor-critic 加分段 GAE，恰执行 steps 步。

    H/V 表的覆盖状态、零初值、softmax(H[s,·]) 稳定采样、每步仅消费
    一个 random.Random(seed) 的 random() 及回合结构均沿
    differential_actor_critic；env、steps、alpha、beta、eta、seed、
    max_steps 的校验亦同。lambda_ 须为非 bool 的 int/float，错型抛
    TypeError；转 float 溢出、非有限或不属 [0, 1] 抛 ValueError。
    全部校验在首次 reset 前完成。

    每步记录采样时的旧概率 p 与旧 V[s]，以当时 R 算
    d=r-R+(0.0 if done else V[s2])-V[s] 并随即 R+=eta*d；H/V 不在
    步内更新。done、单回合满 max_steps 或总步耗尽即结束一段：段末
    令 A=0.0，对本段逆序作 A=d+lambda_*A 并逐位保存（段界切断递推，
    终止不自举、截断与总步末尾照常自举已由各 d 体现），再正序以所存
    p 同步作 H[s,b]+=alpha*A*(I[b=a]-p[b])，随后 V[s]+=beta*A。
    d、A、R 或新 H/V 非有限即抛 ValueError。

    返回键依次为 h、v、average_reward、episodes，结构沿
    differential_actor_critic。同参同 seed 逐值一致，全部随机性
    来自一个 random.Random(seed)。仅用标准库，不引入命令行入口。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(steps, bool) or not isinstance(steps, int):
    ...
    try:
        ...
        raise ValueError(
            "lambda_ must be finite and in [0, 1]"
        ) from exc
    if (
    ...
    def _flush_segment():
        advantages = [0.0] * len(segment)
        advantage = 0.0
        for index in range(len(segment) - 1, -1, -1):
            advantage = segment[index][4] + lambda_value * advantage
            if not math.isfinite(advantage):
                raise ValueError("advantage must remain finite")
            advantages[index] = advantage
        for record, adv in zip(segment, advantages):
            seg_state, seg_action, seg_probs, _value, _delta = record
            new_h_values = {}
            for b, p_b in zip(actions, seg_probs):
                indicator = 1.0 if b == seg_action else 0.0
                new_h = h[(seg_state, b)] + alpha * adv * (indicator - p_b)
                if not math.isfinite(new_h):
                    raise ValueError("H value must remain finite")
                new_h_values[b] = new_h
            new_v = v[seg_state] + beta * adv
            if not math.isfinite(new_v):
                raise ValueError("V value must remain finite")
            for b in actions:
                h[(seg_state, b)] = new_h_values[b]
            v[seg_state] = new_v
    ...
    for _ in range(steps):
        ...
        if not math.isfinite(new_average_reward):
            raise ValueError("average reward must remain finite")
        average_reward = new_average_reward
        segment.append((state, action, probs, value, delta))
        if done or episode_steps >= max_steps:
            episode_results.append([episode_steps, total_reward, done])
            _flush_segment()
            segment = []
...
def ac_until(
    ...
    """在线一步 actor-critic 训练至 H/V 快照收敛或达回合上限。

    训练完全等同 actor_critic(env, E, a, b, g, seed, M)：同一
    random.Random(seed)，每步仅调用一次 random()，done 不自举，
    达步限截断的末步照常自举，H/V 更新次序一致。env 须为
    GridWorld；E、seed、M、K、P 须为非 bool int（除 seed 外均须
    为正），a、b、g、pt、vt 须为非 bool int/float；错型抛
    TypeError。a、b 须有限且属 (0, 1]，g 须有限且属 [0, 1)，
    pt、vt 转 float 溢出、非有限或为负抛 ValueError。全部校验在
    首次 reset 前完成。

    快照为键序恰为 h、v 的 dict，行结构沿用 actor_critic 返回表：
    训练前先存一份全零表；此后每 K 回合及最终回合（第 E 回合）
    完成后各存一份当前 H/V 的独立副本。快照达两项后，每次存入即
    调用 actor_critic_convergence(snapshots, pt, vt, P)；首次
    converged 为真即停止训练，否则跑满 E 回合。δ 或更新后的 H/V
    出现非有限值即抛 ValueError，且不返回部分结果。

    返回键依次为 h、v、episodes、snapshots、report；实际完成 k
    回合时，前三项逐值等于 actor_critic(env, k, a, b, g, seed, M)
    的对应结果，report 为末次 actor_critic_convergence 的完整结果。
    同参同 seed 逐值一致。仅用标准库，不引入命令行入口。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(E, bool) or not isinstance(E, int):
    ...
    for episode in range(E):
        ...
        for _ in range(M):
            ...
            cumulative = 0.0
            action = "L"
            for act, p_act in zip(actions, probs):
                cumulative += p_act
                if cumulative > u:
                    action = act
                    break
            ...
            for act in actions:
                indicator = 1.0 if act == action else 0.0
                new_h = h[(state, act)] + a * delta * (
                    indicator - prob[act]
                )
                if not math.isfinite(new_h):
                    raise ValueError("H value must remain finite")
        ...
        if ran % K == 0 or ran == E:
            ...
            if len(snapshots) >= 2:
                report = actor_critic_convergence(
                    snapshots, policy_tol, value_tol, P
                )
                if report["converged"]:
                    break
...
def nstep_td_prediction(
    ...
    除 n_steps 外，参数校验、异常分类、policy 格式及 URDL 采样均同
    td_lambda_prediction：policy 键恰为从 S 可达的非 G 格，值为 URDL 序
    四项 list；每步仅调用一次 random()，按累计概率取首个严格大于样本的
    动作，未命中取 L。n_steps 须为非 bool 正 int，错型抛 TypeError，
    非正抛 ValueError。

    V 覆盖从 S 可达的全部格（含 G），按坐标升序、初值均为 0.0，G 恒
    0.0。每回合 reset、清空 FIFO，单回合最多 max_steps 步。step 得
    (s2, r, done) 后存 [s, r, done, s2]：队长达 n_steps 即以队头起
    m=n_steps 项更新队头并弹出；回合结束（done 或步限）后按队头冲刷，
    每次 m 取当前队长。G=Σ(k=0..m-1) gamma^k*r[k]，末项未 done 再加
    gamma^m*V[s2]，V 取更新当时值：终止尾不自举，max_steps 截断尾仍
    自举。仅作 V[s]+=alpha*(G-V[s])，G 或新 V 溢出/非有限即抛
    ValueError。全部随机性来自一个 random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    def update(queue, m):
        ...
        if not tail[2]:
            g += discount * v[tail[3]]
        if not math.isfinite(g):
            raise ValueError("n-step return G must remain finite")
        state = queue[0][0]
        new_value = v[state] + alpha * (g - v[state])
        if not math.isfinite(new_value):
            raise ValueError("V value must remain finite")
        v[state] = new_value
...
def lspi(
    ...
    bool 的正 int，错型抛 TypeError，非正抛 ValueError，全部校验先于
    首次 reset。

    从 S 可达的非 G 格按坐标升序排列，初始策略每格为 URDL 序的
    [0.25, 0.25, 0.25, 0.25]。第 i 轮（0 起）以当前策略与 seed+i
    调用 lstdq 得 q，对每格按 URDL 序取 q 值首个最大的动作，同步
    生成 one-hot float 新策略；若新策略与当前策略逐格相同则立即返回。
    跑满 max_iterations 轮策略仍在变化则抛 RuntimeError。lstdq 抛出
    的异常原样透传，且不留任何部分结果。

    返回键序为 policy、q、iterations、steps 的 dict：policy 为按坐标
    升序的 {cell: 四项 URDL float 概率}；q 沿用 lstdq 的键序与值；
    iterations 为已评估轮数，steps 为各轮 lstdq step 数之和，均为
    int。同参同 seed 逐值一致，仅用标准库。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    for i in range(max_iterations):
        ...
        for state in states:
            ...
            row = [0.0, 0.0, 0.0, 0.0]
            row[best_index] = 1.0
            next_policy[state] = row

        unchanged = all(
            next_policy[state] == policy[state] for state in states
        )
        policy = next_policy
        if unchanged:
            return {
    ...
    raise RuntimeError(
        "policy did not stabilize within max_iterations"
    )
...
def gtd2(
    ...
    """按行为策略采样、对目标策略做离策略 GTD2(λ) 评估，返回 V 与 W 表。

    behavior、target 为 dict，键恰为从 S 可达的非 G 格（二元非 bool 整数
    tuple），值为 U/R/D/L 序的四项 list，各项为有限非负 float 且行和为
    1.0；target 取正概率处 behavior 也必须为正。余参数、策略校验、异常、
    URDL 采样、随机消费及回合边界均同 off_policy_td。beta 含 bool 或非
    int/float 抛 TypeError；转 float 溢出、非有限或不属 (0, 1] 抛
    ValueError，全部校验先于 reset。

    V、W 仅覆盖从 S 可达的非 G 格，按坐标升序、初值均为 0.0；每回合
    reset，E 清零，至 done 或 max_steps 步。每步仅调用一次 random()，
    按 behavior 的 URDL 累计概率取首个严格大于样本的动作，未命中取 L。
    step 前令 rho=target[s][a]/behavior[s][a]；step 得 (s2, r, done)
    后，全部求和按坐标序从 0.0 累加，并以旧表令 f[x]=I[x=s]、
    n[x]=I[未 done 且 x=s2]、delta=r+gamma*Σn[x]V[x]-V[s]、
    E[x]=rho*(f[x]+gamma*lambda_*E[x])、u=ΣE[x]W[x]、w=W[s]，再同步作
    V[x]+=alpha*u*(f[x]-gamma*(1-lambda_)*n[x])、
    W[x]+=beta*(delta*E[x]-w*f[x])。rho、delta、E、u、w 或新 V/W 值非
    有限即抛 ValueError。全部随机性来自一个 random.Random(seed)。

    返回键序 values、weights，均为按坐标升序排列的 float 新 dict；同参
    同 seed 逐值一致，不修改行为与目标策略。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    try:
        ...
        raise ValueError("beta must be finite and in (0, 1]")
    if not math.isfinite(beta_value) or beta_value <= 0 or beta_value > 1:
        raise ValueError("beta must be finite and in (0, 1]")
    if not _is_finite_number(gamma) or gamma < 0 or gamma >= 1:
        raise ValueError("gamma must be finite and in [0, 1)")
    if not _is_finite_number(lambda_) or lambda_ < 0 or lambda_ > 1:
    ...
    v = {cell: 0.0 for cell in states}
    weights = {cell: 0.0 for cell in states}
    rng = random.Random(seed)
    decay = gamma * lambda_
    replacement = gamma * (1.0 - lambda_)
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            next_indicator = {}
            for cell in states:
                present[cell] = 1.0 if cell == state else 0.0
                next_indicator[cell] = (
                    1.0 if not done and cell == next_state else 0.0
                )

            bootstrap = 0.0
            for cell in states:
                bootstrap += next_indicator[cell] * v[cell]
            if not math.isfinite(bootstrap):
                raise ValueError("bootstrap must remain finite")
            delta = reward + gamma * bootstrap - v[state]
            if not math.isfinite(delta):
                raise ValueError("delta must remain finite")
            ...
            for cell in states:
                value = rho * (
                    present[cell] + decay * trace[cell]
                )
                if not math.isfinite(value):
                    raise ValueError("eligibility trace must remain finite")
                new_trace[cell] = value

            u = 0.0
            for cell in states:
                u += new_trace[cell] * weights[cell]
            if not math.isfinite(u):
                raise ValueError("u must remain finite")
            w_state = weights[state]
            if not math.isfinite(w_state):
                raise ValueError("W value must remain finite")

            new_v = {}
            new_weights = {}
            for cell in states:
                v_value = v[cell] + alpha * u * (
                    present[cell] - replacement * next_indicator[cell]
                )
                if not math.isfinite(v_value):
                    raise ValueError("V value must remain finite")
                w_value = weights[cell] + beta_value * (
                    delta * new_trace[cell] - w_state * present[cell]
                )
                if not math.isfinite(w_value):
                    raise ValueError("W value must remain finite")
                new_v[cell] = v_value
                new_weights[cell] = w_value
            trace = new_trace
            v = new_v
            weights = new_weights
    ...
    return {
        "values": {cell: float(v[cell]) for cell in states},
        "weights": {cell: float(weights[cell]) for cell in states},
    }
...
def off_policy_mc_prediction(
    ...
    """按行为策略采样、对目标策略做离策略 MC 预测（加权重要性采样）。

    behavior、target 及 episodes、gamma、seed、max_steps 的校验与异常
    均同 off_policy_td。V、C 覆盖从 S 可达的全部格（含 G），按坐标
    升序、初值均 0.0，G 格恒为 0.0。仅用标准库；全部随机性来自一个
    random.Random(seed)；每回合 reset，至 done 或 max_steps 步。

    每步恰调用一次 random()，按 behavior 的 URDL 累计概率取首个严格
    大于样本的动作，未命中取最后一个正概率动作；逐步记录 state、
    reward 与 rho=target[state][a]/behavior[state][a]。回合结束
    （终止与截断均不自举）令 G=0.0、W=1.0，逆序对每项先作
    G=reward+gamma*G、W*=rho；W 为 0 即停止回溯，否则作
    C[state]+=W、V[state]+= W/C[state]*(G-V[state])。rho、G、W、
    C 或 V 溢出或非有限即抛 ValueError。

    返回键序 values、weights、episodes：前两项为 float 字典；
    episodes 为每回合 [steps, total_reward, done]，依次
    int/int/bool，total_reward 从 0 按步累加。同参同 seed 结果一致，
    不修改行为与目标策略。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    v = {cell: 0.0 for cell in all_states}
    c = {cell: 0.0 for cell in all_states}
    history = []
    rng = random.Random(seed)
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            if not math.isfinite(rho):
                raise ValueError("importance ratio must remain finite")
            next_state, reward, done = env.step(actions[action_index])
            trajectory.append((state, reward, rho))
            steps += 1
            total_reward += reward
            if done:
                break
        ...
        g_return = 0.0
        weight = 1.0
        for cell, reward, rho in reversed(trajectory):
            g_return = reward + gamma * g_return
            if not math.isfinite(g_return):
                raise ValueError("return must remain finite")
            weight *= rho
            if not math.isfinite(weight):
                raise ValueError("weight must remain finite")
            if weight == 0.0:
                break
            c[cell] += weight
            if not math.isfinite(c[cell]):
                raise ValueError("C value must remain finite")
            v[cell] += weight / c[cell] * (g_return - v[cell])
            if not math.isfinite(v[cell]):
                raise ValueError("V value must remain finite")
        history.append([steps, total_reward, done])
...
def off_policy_mc_converge(
    ...
    """离策略 MC 预测带固定种子收敛早停，返回键序 values、weights、
    episodes、checks、converged 的 dict。

    env、behavior、target 及 episodes、gamma、seed、max_steps 的校验、
    异常分类、策略采样、加权重要性采样更新、终止/截断处理与随机数消费
    均沿用 off_policy_mc_prediction。check_every、patience 须为非
    bool 的正 int，tolerance 须为非 bool 的 int/float；类型违约抛
    TypeError，check_every、patience 非正、tolerance 转 float 溢出、
    非有限或为负抛 ValueError。全部校验在首次 reset 前完成。
    ...
    episodes 逐值等同 off_policy_mc_prediction 在 episodes=k 与相同
    其余参数下的结果。仅用标准库，不引入命令行入口。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    if isinstance(gamma, bool) or not isinstance(gamma, (int, float)):
        raise TypeError("gamma must be an int or float")
    if isinstance(check_every, bool) or not isinstance(check_every, int):
        raise TypeError("check_every must be an int")
    if isinstance(patience, bool) or not isinstance(patience, int):
        raise TypeError("patience must be an int")
    if isinstance(tolerance, bool) or not isinstance(
    ...
    checks = []
    snapshot = {cell: 0.0 for cell in all_states}
    streak = 0
    converged = False
...
def emphatic_actor_critic(
    ...
    """强调（emphatic）离策略在线一步 actor-critic，目标策略为 softmax
    偏好 H、behavior 为行为策略，返回 h/v 表与逐回合统计。

    behavior 契约、episodes/alpha/beta/gamma/seed/max_steps 的校验，
    H/V 键域与零初值，reset 与 done/max_steps 边界，每步一次 random()
    的随机消费，以及返回契约均同 off_policy_actor_critic；lambda_ 与
    interest 契约同 emphatic_td_lambda：lambda_ 为非 bool 的有限
    int/float 且 ∈ [0, 1]；interest 为 dict，键恰为从 S 可达的非 G
    格（二元非 bool 整数 tuple），值为非 bool 的有限 int/float 且均
    ≥ 0，至少一项 > 0。错型抛 TypeError，其余违约抛 ValueError。

    每回合置 F=ρprev=0.0。每步先以旧 H 按稳定 softmax 得 URDL 概率
    p，再令 F=interest[s]+gamma*ρprev*F、
    M=lambda_*interest[s]+(1-lambda_)*F；随后仅调用一次 random() 按
    behavior 的 URDL 累计概率取首个严格大于样本的动作（未命中取 URDL
    中最后一个正概率动作），令 ρ=p[a]/behavior[s][a]。step 得
    (s2, r, done)：done 时 δ=r-V[s]，否则
    δ=r+gamma*V[s2]-V[s]，达到步限且未 done 的末步同样按后式自举。
    先以旧 p 对各 b 同步作 H[s,b]+=alpha*ρ*M*δ*(I[b=a]-p[b])，再作
    V[s]+=beta*ρ*M*δ。F、M、ρ、δ、梯度中间量或任一更新值非有限即抛
    ValueError。done 即停，否则置 s,ρprev=s2,ρ。全部随机性来自一个
    random.Random(seed)。
...
def wdr_bootstrap(data, gamma=0.9, n=1000, level=0.95, seed=0) -> dict:
    """WDR 点估计的自助法置信区间，对有放回重抽样复用既有评估。

    data 与 gamma 的校验及异常完全沿用 weighted_doubly_robust，
    不修改 data。n、seed 须为非 bool 的 int，错型抛 TypeError，
    n<=0 抛 ValueError。level 须为非 bool 的 int/float，错型抛
    TypeError；转 float 溢出、非有限或不在 (0, 1) 抛 ValueError。

    点估计取 weighted_doubly_robust(data, gamma) 的 estimate。
    用 random.Random(seed)，令 m=len(data)，每轮按序调用 m 次
    randrange(m) 组成有放回样本交既有函数，共 n 轮，samples 按
    生成序保存 float estimate。令 x 为 samples 升序副本、
    a=(1-level)/2，位置为 a*(n-1)、(1-a)*(n-1)；位置 p 为整数
    时取 x[p]，否则令 l=floor(p)、u=ceil(p)，取
    x[l]+(x[u]-x[l])*(p-l)。区间端点非有限抛 ValueError。

    返回新 dict，键序 estimate、interval、samples：estimate 为
    float 点估计；interval 为 [lower, upper] 两个 float 的新
    list；samples 为 float 新列表。同输入同 seed 逐值一致。
    """
    if isinstance(n, bool) or not isinstance(n, int):
        raise TypeError("n must be an int")
    if n <= 0:
    ...
    estimate = weighted_doubly_robust(data, gamma)["estimate"]

    rng = random.Random(seed)
    m = len(data)
    samples = []
    for _ in range(n):
        sample = [data[rng.randrange(m)] for _ in range(m)]
        samples.append(weighted_doubly_robust(sample, gamma)["estimate"])
