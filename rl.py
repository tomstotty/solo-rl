...
def actor_critic(
    ...
    H 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，V 覆盖同样的格子，
    初始均为 0.0。每回合 reset，至 done 或 max_steps 步；每步按
    softmax(H[s,·]) 采样动作，仅调用一次 random()。每步在 step 前
    保留动作概率 p 与旧 V[s]；done 时 delta=r-V[s]，否则
    delta=r+gamma*V[s2]-V[s]（步限截断且未 done 的末步同样自举），
    随即先对各 b 同时作 H[s,b]+=alpha*delta*(I[b=a]-p[b])，
    再 V[s]+=beta*delta。全部随机性来自一个 random.Random(seed)。
