...
def retrace_actor_critic(
    ...
    behavior 单侧行为策略契约、episodes、alpha、beta、gamma、seed、
    max_steps 的校验，H/V 键域与初值，reset 与 done/max_steps 边界，
    以及返回契约均同 vtrace_actor_critic。lambda_ 为非 bool 的
    int/float，错型抛 TypeError；转 float 溢出、结果非有限或不在
    [0, 1] 抛 ValueError。
    ...
    （故 behavior[s][a] 必为正）。step 得 (s2, r, done)，记录 s、a、
    r、p、b=behavior[s][a]、v=V[s]、n=0.0 if done else V[s2] 及
    term=done；done 即停，截断末步仍以 V[s2] 自举。回合末将各步的
    [r, v, n, log(b), log(p[a]), term] 组成 list 序列交给
    retrace(transitions, gamma, lambda_) 取得 targets 与 advantages，
    再按原步序逐 t 令 rho=min(1.0, exp(log(p[a])-log(b)))，先以所存
    p 对各 x 同步作 H[s,x]+=alpha*rho*advantages[t]*(I[x=a]-p[x])，
    再作 V[s]+=beta*(targets[t]-v)。softmax、对数、指数比、优势
    递推中间量或任一更新值非有限（含指数上溢）即抛 ValueError。
    全部随机性来自一个 random.Random(seed)。
    ...
    for _ in range(episodes):
        ...
        for record in trajectory:
            ...
            if not math.isfinite(log_b) or not math.isfinite(log_p):
                raise ValueError("log probabilities must remain finite")
            transitions.append(
                [record[2], record[6], record[7], log_b, log_p, record[8]]
            )
        ...
        for t, record in enumerate(trajectory):
            st, action, _reward, probs, p_a, b_prob, old_v, _n, _term = (
                record
            )
            log_diff = math.log(p_a) - math.log(b_prob)
            if not math.isfinite(log_diff):
                raise ValueError("log-prob difference must remain finite")
            try:
                ratio = math.exp(log_diff)
            except OverflowError:
