...
def lstdq(
    ...
    """按给定策略做 LSTD(λ) Q 版最小二乘策略评估，返回 Q 表与转移数。

    参数合法域、校验顺序、异常分类、reset 前全验、policy 采样与随机消费均同
    lstd，不修改 policy。将从 S 可达的非 G 格按坐标升序编号 0..n-1，并按
    U,R,D,L 将 (s,a) 编号为 0..4n-1：A 为 4n×4n、b 与 z 为 4n 项 0.0，
    phi 为当前 (s,a) 编号的一热向量。每回合 reset 后 z 清零，至 done 或
    max_steps 止。每步 step 得 (s2, r, done) 后作
    z=gamma*lambda_*z+phi；psi 在 done 时全 0，否则仅 s2 的四项取
    policy[s2]（U,R,D,L），再按行列序累加
    A[i][j]+=z[i]*(phi[j]-gamma*psi[j])、b[i]+=z[i]*r；终止（done）
    不自举，截断末步仍自举。随后 A 对角加 ridge，按 lstd 相同主元与
    Gauss-Jordan 规则求解 Aq=b，零主元抛 RuntimeError。迹、A、b、消元量
    或输出溢出/非有限抛 ValueError。返回 {"q", "steps"}：q 为按坐标升序
    ×U,R,D,L 插入序的 ((r,c),action)->float 新字典；steps 为 env.step
    调用总数。仅用标准库。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    dimension = 4 * n
    index = {state: i for i, state in enumerate(states)}
    matrix_a = [[0.0 for _ in range(dimension)] for _ in range(dimension)]
    vector_b = [0.0 for _ in range(dimension)]
    trace = [0.0 for _ in range(dimension)]
    rng = random.Random(seed)
    steps = 0
    decay = gamma * lambda_
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            action = "L"
            action_offset = 3
            for action_offset, (candidate, probability) in enumerate(
            ...
            if not math.isfinite(reward):
                raise ValueError("reward must remain finite")
            sa = 4 * index[state] + action_offset
            for i in range(dimension):
                trace[i] *= decay
            trace[sa] += 1.0
            for i in range(dimension):
                if not math.isfinite(trace[i]):
                    raise ValueError("eligibility trace must remain finite")
            phi = [0.0 for _ in range(dimension)]
            psi = [0.0 for _ in range(dimension)]
            phi[sa] = 1.0
            if not done:
                base = 4 * index[next_state]
                probabilities = policy[next_state]
                for k in range(4):
                    psi[base + k] = probabilities[k]
            for i in range(dimension):
                zi = trace[i]
                row_a = matrix_a[i]
                for j in range(dimension):
                    row_a[j] += zi * (phi[j] - gamma * psi[j])
    ...
    augmented = [
        matrix_a[i] + [vector_b[i]] for i in range(dimension)
    ]
    for k in range(dimension):
        ...
        for i in range(dimension):
            ...
            for j in range(k, dimension + 1):
                ...
                if not math.isfinite(augmented[i][j]):
                    raise ValueError("matrix entries must remain finite")
    weights = [augmented[i][dimension] for i in range(dimension)]

    q = {}
    for cell in states:
        base = 4 * index[cell]
        for k, action_name in enumerate(actions):
            value = float(weights[base + k])
            if not math.isfinite(value):
                raise ValueError("q values must remain finite")
            q[(cell, action_name)] = value
