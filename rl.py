...
def nstep_q_sigma(
    ...
    Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    每回合 reset、清空 FIFO 并先选动作；单回合最多 max_steps 步。
    每次选动作时按选前 Q 保存 ε 贪心策略 π2：URDL 中首个最大 Q 动作
    概率为 1-epsilon+epsilon/4，其余各 epsilon/4。step 得
    (s2, r, done)：done 时 a2、π2 均为 None 且不再选动作，否则立即
    选 a2（步限末步也选）并记其策略 π2；将
    [s, a, r, done, s2, a2, π2] 入队。队长达 n_steps 即更新队头并
    弹头；回合结束后冲刷，m 取当前队长。取前 m 项，末项 done 时令
    G=0.0，否则令 G=Q[s2,a2]（取本次更新时值），再从末项逆推：
    done 项令 G=r，否则自 0.0 按 URDL 累加
    S=Σ(b!=a2) π2[b]*Q[s2,b]，令
    G=r+gamma*((1-sigma)*S+(sigma+(1-sigma)*π2[a2])*G)。作
    Q[s,a]+=alpha*(G-Q[s,a])，仅更新队头；G 或新 Q 非有限抛
    ValueError。全部随机性来自一个 random.Random(seed)。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    ...
    def update(queue, m):
        """以队头起 m 项逆推 n 步 Q(σ) 回报，仅更新队头的 (s, a)。"""
        tail = queue[m - 1]
        if tail[3]:
            g = 0.0
        else:
