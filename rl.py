...
def annealed_boltzmann_q(
    ...
    """温度线性退火的 Boltzmann(softmax) Q-learning，返回
    {((row, col), action): float}。

    Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    每回合 reset，单回合最多 max_steps 步，末步未终止仍自举。
    全部随机性来自一个 random.Random(seed)，每步仅调用一次
    random()，不调用 randrange()。

    第 e 回合（0 起）的温度 T：e>=decay_episodes 时 T=temp_end，
    否则令 t=e/decay_episodes、
    T=temp_start+(temp_end-temp_start)*t。每步按 URDL 令
    m=max_a Q[s,a]、w[a]=exp((Q[s,a]-m)/T)、z=sum(w,0.0)，取
    u=rng.random()*z，从 0.0 按 URDL 累加 w 并选择首个累计值严格
    大于 u 的动作，未命中取 L。step 后 done 时目标为 r，否则为
    r+gamma*max_a Q[s2,a]，作 Q[s,a]+=alpha*(目标-Q[s,a])；
    新 Q 非有限抛 ValueError。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
