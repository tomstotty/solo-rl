...
def ppo_minibatch(
    ...
    logits、batch、lr、c 的校验与异常逐项沿用 ppo_update；batch_size、
    epochs、seed 须为非 bool 的 int，类型不符抛 TypeError，其中
    batch_size、epochs <= 0 抛 ValueError。全部校验先于任何修改完成，
    且不修改输入。仅建一个 random.Random(seed)，并复制 logits 为 L。
    每轮令 p=list(range(len(batch)))，按 i 自 n-1 降至 1 取
    j=rng.randrange(i+1) 并交换 p[i]、p[j] 完成洗牌；再按 p 连续切为
    至多 batch_size 项的小批，尾批保留。依 p 取 batch 行组成小批，
    逐批调用 ppo_update(L, sub, lr, c, 1) 并以返回 logits 续训；任一批
    失败均原样抛出且不返回部分结果。logits 为最终 L；objectives 为按轮
    排列的各小批 objectives[0] float 列表；probabilities 为最终 L 的
    稳定 softmax 二维 float 列表。同输入同 seed 逐值一致。
    """
    if not isinstance(logits, list):
        raise TypeError("logits must be a list")
    if not logits:
    ...
    for _ in range(epochs):
        ...
        for start in range(0, n_batch, batch_size):
            ...
            sub = [batch[index] for index in indexes]
            result = ppo_update(L, sub, lr, c, 1)
            L = result["logits"]
            objectives.append(float(result["objectives"][0]))
