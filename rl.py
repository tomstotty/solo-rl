...
def convergence_report(
    ...
    """按逐回合 reward 的连续滑窗均值判定收敛。

    episode_results 每项须为 [steps, reward, done]：steps 为非 bool
    正 int，reward 为可转为有限 float 的非 bool int/float，done 为
    bool。不修改 episode_results。

    对每个长度为 window 的完整连续滑窗按
    sum(map(float, r), 0.0) / window 求均值，依序得 M。取最小
    j >= patience，使 abs(M[k] - M[k-1]) <= tolerance 对
    k = j-patience+1..j 均成立。返回键依次为 converged、episode、
    reward_mean、windows；命中时分别为 True、window+j（从 1 起的
    回合号）、M[j]、M，否则为 False、None、None、M。
    """
    ...
    for row in episode_results:
        if not isinstance(row, list):
            raise TypeError("each episode result must be a list")
        if len(row) != 3:
            raise ValueError("each episode result must have three items")
    ...
    means = [
        sum(map(float, rewards[i : i + window]), 0.0) / window
