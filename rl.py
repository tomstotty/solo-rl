...
def watkins_q_lambda(
    ...
    """Watkins 的 Q(λ)，返回 {((row, col), action): float}。

    Q 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，初始为 0.0。
    每回合 reset，资格迹 E 清零，先选动作；单回合最多 max_steps 步。
    全部随机性来自一个 random.Random(seed)。

    非终止步先取下一状态上 U/R/D/L 序首个最大 Q 动作 g，再 ε 贪心
    选 a2；更新全部 Q 后，a2 == g 时迹乘以 gamma*lambda_，否则清零。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
    ...
    def apply_update(delta, eligibility):
        for key in q:
            q[key] += alpha * delta * eligibility[key]
            if not _is_finite_number(q[key]):
                raise ValueError("update produced a non-finite value")
    ...
    for _episode in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            if done:
                delta = reward - q[(state, action)]
                apply_update(delta, eligibility)
                break
