...
def awr_until(
    ...
    threshold、patience 的校验与异常沿用 ppo_success_streak 同名参数，
    全部校验在首次 reset 前完成。以 advantage_weighted_train 默认训练
    超参（alpha=0.05、gamma=0.9、lambda_=0.95、temp=1.0、cap=20.0、
    epochs=4）及给定 seed、max_steps 连续训练：采样、GAE 递推、H/V
    更新、随机消费及终止/截断处理逐项相同，全部随机性来自一个
    random.Random(seed)。

    每次完成整回合采样和更新后，以该回合末步 done 为成功（步限截断记
    False）加入成功序列，并调用 ppo_success_streak(successes, window,
    threshold, patience)；首次 converged 为真即停止，否则训练
    max_episodes 回合。

    返回键依次为 h、episodes、objectives、weights、report；实际完成
    k 回合时，前四项逐值等于 advantage_weighted_train(env, episodes=k,
    seed=seed, max_steps=max_steps) 的对应返回，report 为成功序列的
    完整 ppo_success_streak 结果。不额外消费随机数，更新器抛出的异常
    原样透传且不返回部分结果，同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
