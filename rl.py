...
def ppo_train_until(
    ...
    """按 ppo_train 默认超参连续训练，成功序列收敛即停，返回训练与收敛结果。

    env、seed、max_steps 的校验与异常同 ppo_train；max_episodes 同其
    episodes 契约（非 bool 正 int）；window、threshold、patience 同
    ppo_success_streak 同名参数契约，全部在采样前校验。以 ppo_train
    默认训练超参（alpha=0.05、gamma=0.9、lambda_=0.95、c=0.2、
    epochs=4）及给定 seed/max_steps 连续训练；每完成一回合的采样与
    更新后，将该回合步列表末项的 done 加入成功序列并调用
    ppo_success_streak，首次 converged 为真即停止，否则训练
    max_episodes 回合。

    返回键依次为 h、episodes、objectives、convergence；实际完成 k
    回合时，前三项逐值等于
    ppo_train(env, episodes=k, seed=seed, max_steps=max_steps)
    的对应返回，convergence 为成功序列的完整 ppo_success_streak
    结果。不额外消费随机数，同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
