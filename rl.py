...
def actor_critic_lambda(
    ...
    """在线资格迹 actor-critic(λ)，返回 h/v 表与逐回合统计。

    H 覆盖从 S 可达的非 G 格与 U/R/D/L 的全部组合，V 覆盖同样的格子，
    初始均为 0.0。每回合 reset，按 H/V 键序将策略迹 EH 与值迹 EV 清零，
    至 done 或 max_steps 步；每步按 softmax(H[s,·]) 稳定采样动作，仅
    调用一次 random()。采样后保留概率 p 与旧 V[s]，step 得
    (s2, r, done)：done 时 δ=r-V[s]，否则 δ=r+gamma*V[s2]-V[s]，达到
    步限且未 done 的末步同样按后式自举。先按键序使所有 EH、EV 乘
    gamma*lambda_，再对 URDL 各 b 作 EH[s,b]+=I[b=a]-p[b]，并作
    EV[s]+=1.0；随后按键序对所有项作 H+=alpha*δ*EH、
    V+=beta*δ*EV。δ、迹或任一更新值非有限抛 ValueError。done 即停。
    全部随机性来自一个 random.Random(seed)。
    ...
    for _ in range(episodes):
        ...
        for _ in range(max_steps):
            ...
            trace_decay = gamma * lambda_
            for key in eh:
                eh[key] *= trace_decay
                if not math.isfinite(eh[key]):
                    raise ValueError("policy eligibility trace must be finite")
            for key in ev:
                ev[key] *= trace_decay
                if not math.isfinite(ev[key]):
                    raise ValueError("value eligibility trace must be finite")
            for b in actions:
                eh[(state, b)] += (
                    1.0 if b == action else 0.0
                ) - prob[b]
            ...
            if not math.isfinite(ev[state]):
                raise ValueError("value eligibility trace must be finite")
            for key in h:
                new_value = h[key] + alpha * delta * eh[key]
                if not math.isfinite(new_value):
                    raise ValueError("H value must remain finite")
                h[key] = new_value
            for key in v:
                new_value = v[key] + beta * delta * ev[key]
                if not math.isfinite(new_value):
