...
def segmented_vtrace(r, v, n, b, p, term, trunc, gamma=0.9, rho_clip=1.0, c_clip=1.0) -> dict:
    ...
    为 bool，同一位置不得两个标记同时为 True，且末项两个标记不得
    同时为 False。gamma、rho_clip、c_clip 为非 bool 的 int/float
    标量，其中 gamma 须在 [0, 1]，rho_clip、c_clip 须 > 0。数值输入
    先复制并转换为 float（转换溢出或结果非有限均抛 ValueError），
    不修改原序列。逐项令 z=exp(p-b)（上溢抛 ValueError，下溢为
    0.0 合法）、rho=min(z,rho_clip)、c=min(z,c_clip)。逆序令
    base=0.0 if term[t] else n[t]、d=rho[t]*(r[t]+gamma*base-v[t])；
    term[t] 或 trunc[t] 时 x[t]=v[t]+d，否则
    x[t]=v[t]+d+gamma*c[t]*(x[t+1]-n[t])。再令 boot=0.0 if
    term[t]、n[t] if trunc[t]、否则 x[t+1]，
    A[t]=rho[t]*(r[t]+gamma*boot-v[t])。对数差、中间量或输出非有限
    均抛 ValueError。values 为 x[0..T-1]，两个列表均按原时序排列为
    float 新 list。
    """
    for name, seq in (
        ("r", r),
        ...
        if not isinstance(seq, (list, tuple)):
            raise TypeError(f"{name} must be a list or tuple")
    if len(r) == 0:
        raise ValueError("r, v, n, b, p, term and trunc must be non-empty")
    if not (len(r) == len(v) == len(n) == len(b) == len(p) == len(term) == len(trunc)):
        raise ValueError(
    ...
    if not (term[-1] or trunc[-1]):
        raise ValueError("the last item must have term or trunc True")
    ...
    for t in range(T - 1, -1, -1):
        ...
        if term[t] or trunc[t]:
            ...
            gap = x_next - next_values[t]
            if not math.isfinite(gap):
                raise ValueError("value gap must be finite")
            x_t = values[t] + d + g * clipped_c[t] * gap
        ...
        if term[t]:
            ...
            boot = x_next
        advantage_delta = rewards[t] + g * boot - values[t]
        if not math.isfinite(advantage_delta):
            raise ValueError("segmented vtrace advantage delta must be finite")
        advantage = rho[t] * advantage_delta
        if not math.isfinite(advantage):
            raise ValueError("advantage must be finite")
        advantages[t] = advantage
        x_next = x_t
    return {"values": x_values, "advantages": advantages}
