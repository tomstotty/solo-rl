...
def ppo_train_minibatch_kl(
    ...
    """带 KL 早停的小批量 PPO 训练（每回合 GAE 采样后多轮小批量截断
    更新），返回 h 表、逐回合步表、目标值、KL 与早停标记。

    除 target_kl 外，参数校验及异常逐项沿用 ppo_train_minibatch，全部
    校验在采样前完成。target_kl 为 bool 或非 int/float 抛 TypeError；
    转 float 溢出、非有限或 <=0 抛 ValueError，校验后记为 t。状态
    编号、采样、GAE、终止/截断、V 更新及返回的 h、episodes 均逐项
    沿用 ppo_train_minibatch。第 e 回合（0 起）以步序
    [s, a, old_logp, A] 为 batch，调用
    ppo_minibatch_kl(H, batch, batch_size, float(alpha), c, epochs,
    t, seed+e)，以其返回 logits 续训；seed+e 只控制该回合洗牌，采样
    随机流不额外消费。返回键依次为 h、episodes、objectives、kls、
    stopped；objectives、kls、stopped 均与回合数等长，第 e 项依次为
    该回合 ppo_minibatch_kl 返回的 objectives 浮点列表、kls 浮点
    列表与 stopped 布尔值。新运算结果非有限均抛 ValueError，失败无
    部分结果；同参同 seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
