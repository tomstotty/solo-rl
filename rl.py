...
def ppo_train_minibatch_adaptive_kl(
    ...
    """自适应 KL 惩罚系数的小批量 PPO 训练（每回合 GAE 采样后多轮
    小批量 KL 惩罚更新），返回 h/v 表、逐回合步表、目标值、KL 与
    惩罚系数。

    除 batch_size、shuffle_seed 外，env、episodes、alpha、gamma、
    lambda_、epochs、seed、max_steps 以及 beta、target、factor、lo、
    hi 的校验与异常逐项沿用 ppo_train_adaptive_kl，全部校验在采样前
    完成。batch_size 须为非 bool 的 int，类型不符抛 TypeError，非正
    抛 ValueError；shuffle_seed 须为非 bool 的 int，类型不符抛
    TypeError。

    状态编号、H/V 初值、每回合 reset、softmax(URDL) 采样且每步仅
    一次 random.Random(seed).random()、GAE 逆推、终止/截断自举、
    V 的步序更新均同 ppo_train_adaptive_kl。记 b=float(beta)，第 e
    回合（0 起）将步序批次 [s, a, old_logp, A] 传给
    ppo_minibatch_adaptive_kl，batch_size、target、factor、lo、hi、
    epochs 同名照传，另传 lr=float(alpha)、beta=b、
    seed=shuffle_seed+e，以其返回 logits 续训，记录该次返回的
    objectives、kls、betas 三组历史，并以 betas 末项续作下一回合
    的 b。采样随机流与洗牌随机流（仅 shuffle_seed+e 控制）彼此
    隔离，更新不额外消费采样随机数。异常直接透传，失败时无部分
    结果。

    返回键依次为 h、v、episodes、objectives、kls、betas；h、v、
    episodes 同 ppo_train_value_clip，后三项均与回合数等长，第 e
    项为该回合 ppo_minibatch_adaptive_kl 返回的完整 float 列表。
    同参同 seed/shuffle_seed 逐值一致。
    """
    if not isinstance(env, GridWorld):
        raise TypeError("env must be a GridWorld")
    if isinstance(episodes, bool) or not isinstance(episodes, int):
